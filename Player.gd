...
onready var dialog_ray = $DialogCollider
onready var shadow = $Shadow
onready var objectsCollider = $ObjectsColider
...
var wayClosedDialogue = [
	{"name": "System", "text": "Ainda não desenvolvido, volte mais tarde!"},
]

var initalDialogue = [
	{"name": "Eu", "text": "Nossa, acho que dormi novamente observando as arvores e o lago."},
	{"name": "Eu", "text": "Depois que terminei minha jornada em kanto não tenho mais nada para fazer."},
	{"name": "Eu", "text": "irei olhar ao redor."},
]
...
func _ready():
	...
	if not Utils.has_saw_first_dialogue:
		stop_input = true
...
func need_to_turn():
	...
	if input_direction.x < 0:
		objectsCollider.position.x = -8
		objectsCollider.position.y = 8
		new_facing_direction = FacingDirection.LEFT
	elif input_direction.x > 0:
		objectsCollider.position.x = 28
		objectsCollider.position.y = 8
...
func initial_message():
	Utils.get_dialogue().start(initalDialogue)

func closed_way_message():
	Utils.get_dialogue().start(wayClosedDialogue)
...
func move(delta):
	...
	if door_ray.is_colliding():
		var doorCollided = door_ray.get_collider()
		if not doorCollided.is_open:
			doorCollided.closed_door_message()
			is_moving = false
			anim_state.travel("Idle")
			return
